...
let carModelsAnonymousFunction = inventory.sort((a,b) => {
    let modelA=a.car_model;
    let modelB=b.car_model;
    if (modelA < modelB){
        return -1;
...
console.log(carModelsAnonymousFunction);
...
const carYearsMapped = inventory.map((car) => {
    return car.car_year;
});
console.log(carYearsMapped);
...
let oldCarsFiltered = inventory.filter((car) => {
    return car.car_year < 2000;
});

console.log(oldCarsFiltered.length);
...
console.table(JSON.stringify(BMWAndAudi));
